...
By using 100,000 random samples from $\Gamma(2,\frac{1}{3})$, we get almost the same result as 6. Why Monte-Carlo method works can be formally proved by using the [Law of Large Numbers](https://people.math.umass.edu/~lr7q/ps_files/teaching/math456/lecture17.pdf). 

&nbsp;

Likewise, as long as we have sufficient amount of samples, we can use Monte-Carlo simulation to make inference about the unknown quantities even if it cannot be derived analytically. This idea can be applied to deriving posterior as well, and the key point is how we can make samples withour knowing the exact probability distribution. Luckily, MCMC has answer to this, which is the very next topic we are going to discuss about.
