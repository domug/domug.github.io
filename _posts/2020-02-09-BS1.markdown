...
Just like any other field of study that is built upon mathematics, Bayesian Statistics is also filled with heavy amount of mathematical formulas and proofs, and I don't want to be overwhelmed by them (because I couldn't understand every single piece of the nitty-gritty mathematics in the textbooks as an undergraduate student...). For this reason, I will try to keep the rigorous mathetical proofs as minimum as possible and rather focus on the insight and the intuition Bayesian statistics can give us.

Hence, we will be focusing on how to implement Bayesian inference using **pymc3**, a renowned PPL (Probabilistic Programming Language) designed to run on python. 
