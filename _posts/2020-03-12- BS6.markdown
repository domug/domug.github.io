...
So in a nutshell, the direction of the HMC sampler is effected by the gradient of the log likelihood, allowing us to get reliable samples that realistically represents the target distribution. This might sound gibbserish at the moment, but let's take a look at the step by step details of HMC algorithm.
